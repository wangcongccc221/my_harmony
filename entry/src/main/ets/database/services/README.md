...
本项目的服务层基于 `createService` 工厂函数构建，提供了三个专用的业务服务类：
...
async function saveFruitInfo(ctx: Context) {
  ...
  const id = await FruitInfoService.save(data, ctx);
...
async function updateFruitInfo(ctx: Context, id: number) {
  const affected = await FruitInfoService.update(id, {
    FruitName: '新苹果',
    BatchWeight: 150.5
  }, ctx);
...
| 方法 | 参数 | 返回值 | 说明 |
|------|------|--------|------|
| `save` | `data, ctx?` | `Promise<number>` | 保存单条记录，返回ID |
| `batchSave` | `list, ctx?` | `Promise<number>` | 批量保存，返回插入数量 |
| `update` | `id, data, ctx?` | `Promise<number>` | 更新记录，返回影响行数 |
| `delete` | `id, ctx?` | `Promise<number>` | 删除记录，返回影响行数 |
| `queryAll` | `ctx?` | `Promise<ESObject[]>` | 查询所有记录 |
| `queryPage` | `page, size, ctx?` | `Promise<ESObject[]>` | 分页查询 |
...
async function saveGradeInfo(ctx: Context) {
  ...
  const id = await GradeInfoService.save(data, ctx);
  console.log(`保存成功，ID: ${id}`);
}
...
async function saveExportInfo(ctx: Context) {
  ...
  const id = await ExportInfoService.save(data, ctx);
  console.log(`保存成功，ID: ${id}`);
}
...
async function batchSaveExportInfo(ctx: Context) {
  ...
  const count = await ExportInfoService.batchSave(dataList, ctx);
  console.log(`批量保存成功，数量: ${count}`);
}
...
所有 Service 都通过 `createService` 工厂函数生成，完全避开 ArkTS 的静态继承限制：
...
const extendedService: ExtendedGradeInfoService = {
  ...BaseGradeInfoService,
  queryByCustomerId: async (ctx, customerId) => { ... }
};
...
### 1. 参数顺序
所有 CRUD 方法的参数顺序为：数据在前，Context 在后（可选）

```typescript
// ✅ 正确
await FruitInfoService.save(data, ctx);
await FruitInfoService.update(id, data, ctx);

// ❌ 错误（旧版本）
await FruitInfoService.save(ctx, data);
```
...
### 5. 类型转换
由于返回类型是 `ESObject[]`，使用时需要进行类型转换：

```typescript
const list = await FruitInfoService.queryAll(ctx);
for (const item of list) {
  const fruitInfo = item as FruitInfo;
  console.log(fruitInfo.FruitName);
}
```
...
### ❌ 不推荐
- 直接在 UI 或 HTTP Handler 中调用 `DatabaseInterface`
- 绕过服务层直接操作数据库
...
├── services/
│   ├── index.ets              # 统一导出
│   ├── createService.ets      # 工厂函数（核心）
│   ├── FruitInfoService.ets   # 水果信息服务（70行）
│   ├── GradeInfoService.ets   # 等级信息服务（131行）
│   ├── ExportInfoService.ets  # 导出信息服务（147行）
│   └── README.md              # 本文档
├── utils/network/database/
│   └── dispatch/
│       └── DatabaseQueueManager.ets  # 底层队列管理器
└── database/
    ├── DatabaseInterface.ets  # 数据库抽象层
    └── models/
...
| Service | 重构前 | 重构后 | 减少 |
|---------|--------|--------|------|
| GradeInfoService | 237 行 | 131 行 | 45% |
| FruitInfoService | 217 行 | 70 行 | 68% |
| ExportInfoService | 266 行 | 147 行 | 45% |
| 总计 | 720 行 | 348 行 | 52% |
