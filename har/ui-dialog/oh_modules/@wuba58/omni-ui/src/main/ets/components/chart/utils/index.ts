...
export function calculateNum (arr, isMin?) {
  ...
  let max = 0
  let min = 0
  let pow; let sum; let step: number; let absLow; let i; let j; let k = 0
  outer:
  for (i = 0; i < 10; i++) {
    pow = Math.pow(10, i)
    for (j = 1; j <= 10; j++) {
...
export function percentToNumer(value): number {
...
export function rotatePoint(x: number, y: number, centerX: number, centerY: number, deltaTheta: number): InterfaceObj {
...
export function getColor (i: number, color: Array<string>): string {
  if (color[i]) {
    return color[i]
  }
  let opacityBase = 1
